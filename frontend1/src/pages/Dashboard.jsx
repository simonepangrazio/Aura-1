      ...
      <aside style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 240, background: 'rgba(255,255,255,0.02)', borderRight: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', padding: '1.5rem 1rem', zIndex: 40 }}>
        ...
        {[
          ...
          <button key={item.id} onClick={() => setTab(item.id)}
            style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'0.75rem 0.875rem', borderRadius:'0.875rem', border:'none', cursor:'pointer', width:'100%', textAlign:'left', fontSize:'0.875rem', fontWeight: tab === item.id ? 600 : 400, marginBottom:4, transition:'all .2s',
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'1rem', padding:'1rem' }}>
          ...
          <button onClick={logout}
            style={{ display:'flex', alignItems:'center', gap:'0.375rem', background:'none', border:'none', cursor:'pointer', color:'#52525b', fontSize:'0.78rem', padding:0, width:'100%' }}
        ...
        {tab === 'services' && (
          <>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'1.25rem' }}>
                        ...
                        {/* Status badge */}
                        <div style={{ display:'flex', alignItems:'center', gap:'0.375rem', background: isRunning ? 'rgba(16,185,129,0.12)' : 'rgba(255,255,255,0.05)', border:`1px solid ${isRunning ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.1)'}`, borderRadius:999, padding:'4px 10px', fontSize:'0.7rem', fontWeight:600, color: isRunning ? '#10b981' : '#52525b', flexShrink:0 }}>
            ...
            {ALL_SERVICES.filter(s => !user.services.includes(s.id)).length > 0 && (
              <>
                <h2 style={{ ...syne, fontSize:'1.25rem', color:'#fff', margin:'3rem 0 1.25rem' }}>Agent disponibili a catalogo</h2>

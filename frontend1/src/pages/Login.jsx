...
export default function Login() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#08080e', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Back to home */}
      <button onClick={() => navigate('/')} style={{ position:'absolute', top:24, left:24, display:'flex', alignItems:'center', gap:'0.5rem', color:'#52525b', fontSize:'0.85rem', background:'none', border:'none', cursor:'pointer' }}
      ...
      <div style={{ width: '100%', maxWidth: 440, position:'relative' }}>
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'1.75rem', padding:'2.5rem', backdropFilter:'blur(20px)' }}>
          ...
          {/* Tabs */}
          <div style={{ display:'flex', background:'rgba(255,255,255,0.04)', borderRadius:'0.875rem', padding:'0.25rem', marginBottom:'2rem' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
            {tab === 'register' && (
              <div>
                <label style={{ fontSize:'0.75rem', color:'#71717a', display:'block', marginBottom:'0.375rem' }}>Nome completo</label>
            ...
            <div>
              <label style={{ fontSize:'0.75rem', color:'#71717a', display:'block', marginBottom:'0.375rem' }}>Email</label>
            ...
            <div>
              ...
              <div style={{ position:'relative' }}>
                ...
                <button onClick={() => setShowPass(!showPass)}
                  style={{ position:'absolute', right:'0.75rem', top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'#52525b', padding:'0.25rem' }}
                >
                  {showPass
            ...
            <button onClick={tab === 'login' ? handleLogin : handleRegister}
              disabled={loading}
              style={{ marginTop:'0.5rem', background:'linear-gradient(135deg,#7c3aed,#0891b2)', border:'none', color:'#fff', fontWeight:700, fontSize:'0.95rem', padding:'1rem', borderRadius:'0.875rem', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, boxShadow:'0 0 30px rgba(124,58,237,0.3)', transition:'opacity .2s' }}
            >
